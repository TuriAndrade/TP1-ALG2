...
class KDTree:
    def __init__(self, points: List[Point]) -> None:
        ...
        self.__points: List[Point] = list(set(points))
        self.__dim: int = self.__points[0].getDim()
        self.__nPoints = len(self.__points)
        self.__height: int = 0
        self.__root: KDNode = self.build(
            self.__points, 0, np.array([[-np.inf, np.inf]] * self.__dim))
    ...
    def build(self, points: List[Point], depth: int, boundingBox: NDArray[np.float32]) -> KDNode:
        ...
        if (currentNPoints == 0):
            ...
            coordinate: int = depth % self.__dim
            ...
            updateBoundingBoxLeft[coordinate][1] = median.getCoordinate(
                coordinate)

            updateBoundingBoxRight = np.copy(boundingBox)
            updateBoundingBoxRight[coordinate][0] = median.getCoordinate(
                coordinate)
    ...
    def kNearestNeighbors(self, target: Point, k: int) -> List[Tuple[Point, float]]:
        ...
        self.kNearestNeighborsHelper(self.__root, target, 0, k, maxHeap)
    ...
    def computeDists(self, target: Point) -> List[Tuple[Point, float]]:
        ...
        self.computeDistsAux(self.__root, target, dists)
    ...
    def searchRange(self, searchInterval: NDArray[np.float32]) -> List[KDNode]:
        reportedNodes: List[KDNode] = []
        self.searchRangeHelper(self.__root, searchInterval, reportedNodes)
